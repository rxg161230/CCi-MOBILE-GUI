...
if (isfield(p,'Left')==1)
    ...
    if strcmp(p.Left.ImplantGeneration, 'CIC4')
        switch (p.Left.StimulationMode)
            ...
            case 'MP1'
                p.Left.StimulationModeCode = 24; %only for CIC4 chip type
            case 'MP2'
                p.Left.StimulationModeCode = 25; %only for CIC4 chip type
            otherwise
...
if (isfield(p,'Right')==1)
    ...
    if strcmp(p.Right.ImplantGeneration, 'CIC4')
        switch (p.Right.StimulationMode)
            ...
            case 'MP1'
                p.Right.StimulationModeCode = 24; %only for CIC4 chip type
            case 'MP2'
                p.Right.StimulationModeCode = 25; %only for CIC4 chip type
            otherwise
